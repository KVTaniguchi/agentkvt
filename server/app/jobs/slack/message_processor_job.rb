module Slack
  class MessageProcessorJob < ApplicationJob
    ...
    def notify(workspace, result)
      ...
      text = "#{result['summary']}#{urgency_tag}"
      Slack::Notifier.call(channel: owner_user_id, text: text, workspace: workspace)
    end

    def append_research(workspace, message, result)
      objective_id = result["objective_id"]
      objective = workspace.objectives.find_by(id: objective_id) if objective_id.present?

      unless objective
        # Fall back to notify if we can't match an objective
